...
- Cross-platform — macOS (launchd), Linux (systemd + nohup fallback), Windows (background process)
- Session persistence — resume conversations across messages

## Prerequisites

- Node.js >= 18
- macOS, Linux, or Windows (requires Git Bash or compatible shell)
...
- **Windows**: runs as a background process (requires Git Bash or compatible shell)
...
- Platform-native service management keeps the daemon running (launchd on macOS, systemd/nohup on Linux, background process on Windows)

## Windows Notes

### Prerequisites
- **Git Bash** or compatible shell environment (Cygwin/MSYS2)
- Node.js >= 18 installed and available in PATH

### Important Limitations
1. **Background Process (not Windows Service)**: The Windows version runs as a background process managed by PID file, not as a native Windows service
2. **Process Lifetime**: The process will stop when the terminal is closed (unless running in background)
3. **No Auto-start**: Does not start automatically on system boot (use PM2 or similar tools for this)

### Persistent Process Management (Recommended)
For production use, we recommend using **PM2** for process management:

```bash
npm install -g pm2
pm2 start dist/main.js --name wechat-claude-code
pm2 save
pm2 startup  # Setup auto-start on boot (requires admin privileges)
```

### File Locations on Windows
- **Data directory**: `C:\Users\<username>\.wechat-claude-code\`
- **Log files**: `C:\Users\<username>\.wechat-claude-code\logs\stdout.log`
- **PID file**: `C:\Users\<username>\.wechat-claude-code\wechat-claude-code.pid`

### Troubleshooting
- If `npm run daemon -- start` fails, ensure you're using Git Bash or compatible shell
- Check logs in `~/.wechat-claude-code/logs/stderr.log` for error details
- If process doesn't start, ensure `dist/main.js` exists (run `npm run build` first)
