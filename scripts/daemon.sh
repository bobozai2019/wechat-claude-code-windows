...
# Supports: macOS (launchd) / Linux (systemd + nohup fallback) / Windows (background process)
# =============================================================================
...
windows_start() {
  ...
  local node_bin="node"  # Assume node is in PATH
  ...
  # Start process in background, redirect output to log files
...
main() {
  ...
  case "$OS_TYPE" in
    ...
    *)
      ...
      echo "Supported platforms: macOS (Darwin), Linux, Windows (MINGW/CYGWIN/MSYS)"
      exit 1
      ;;
  esac
}
